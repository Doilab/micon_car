...
if(mode==MODE_P)//P補償
{
   kp=0.2655;//0.00531*0.5;//限界感度法の値に基づいて設定する
}
if(mode==MODE_PI)//PI補償
{
   kp=0.23895;//0.00531*0.45;//限界感度法の値に基づいて設定する
   ti=0.21083;//0.253/1.2;//限界感度法の値に基づいて設定する
}
if(mode==MODE_PID)//PID補償
{
    kp=0.003186;//0.00531*0.6;//限界感度法の値に基づいて設定する
    ti=0.1265;//0.253*0.5;//限界感度法の値に基づいて設定する
    td=0.031625;//0.253/8;//限界感度法の値に基づいて設定する
}
if(mode==MODE_ORIGINAL)//各班独自の補償
{
  //ここに独自に考えた補償パラメータを書く（制御則もこの後に追記する）
  ;
}
